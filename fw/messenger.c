...
// write a message to be sent out from the radio
void messenger_send(const uint8_t *src_buf, unsigned src_len) {
  uint32_t enc_len, bytes_remaining, total_bytes_sent;
  size_t bytes_sent;
  ...
  while (bytes_remaining) {
    ...
    // update for the UART2 code, return code is not the bytes sent but rather
    // the status code
    int status = UART2_write(uart, b64_buf + total_bytes_sent, bytes_remaining,
                             &bytes_sent);
    if (status < 0)
